...
namespace MedisoftFhirAgent.Repositories
{
    public class MessageOutBoundRepository : IMessageOutBoundRepository<Patient> 
    {
        ...
        public async Task<MessageQueueOutBound> sendToIntegrationAsync(List<Patient> listOfObject)
        {
            ...
            {
                foreach (var pt in listOfObject)
                {
                    if (pt.birthDate != "")
                    {
                        DateTime birthDate = DateTime.Parse(pt.birthDate);
                        pt.birthDate = String.Format("{0:yyyy-MM-dd}", birthDate);
                    }
                }
                ...
                using (var httpClientHandler = new HttpClientHandler())
                {
                    ...
                    using (var client = new HttpClient(httpClientHandler))
                    {
                        ...
                        var url = "https://yeatspatientportal-integration-staging.azurewebsites.net/MessageQueue/push";
        ...
        public async Task<MessageQueueOutBound> sendDeletedToIntegrationAsync(List<Patient> listOfObject)
        {
            if (listOfObject != null)
            {
                foreach (var pt in listOfObject)
                ...
                msgQueue.Source = "MediSoft";
                msgQueue.Status = "";
                msgQueue.ResourceType = "Deleted Patients";
                msgQueue.Type = "I";
                ...
                using (var httpClientHandler = new HttpClientHandler())
                {
                    ...
                    using (var client = new HttpClient(httpClientHandler))
                    {
                        ...
                        {
                            _lgc.Log("Patient_deleted_sent_", result.ToString());
                        }
                    }
                }
